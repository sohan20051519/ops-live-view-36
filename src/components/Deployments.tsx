...
const Deployments = () => {
  ...
  const fetchBranches = async (url: string, token?: string) => {
    ...
    try {
      ...
      const urlPattern = /(?:https?:\/\/)?(?:www\.)?(?:github\.com|gitlab\.com|bitbucket\.org)\/([^\/]+)\/([^\/]+?)(?:\.git)?(?:\/.*)?$/;
      const match = url.match(urlPattern);
      
      if (!match) {
        toast({
          title: "Invalid Repository URL",
          description: "Please provide a valid repository URL",
          variant: "destructive",
        });
        return;
      }
      ...
      // Determine API URL based on git provider
      if (url.includes("github.com")) {
        ...
        // For other providers, show default branches
      ...
      if (!response.ok) {
        if (response.status === 404) {
          toast({
            ...
            description: "The repository might be private or doesn't exist. Try adding a personal access token.",
            variant: "destructive",
          });
        } else {
      ...
      if (url.includes("github.com")) {
        branchNames = branches.map((branch: any) => branch.name);
      } else if (url.includes("gitlab.com")) {
        branchNames = branches.map((branch: any) => branch.name);
      }
      ...
      if (branchNames.length > 0) {
        // Set default branch priority: main > master > first available
        const defaultBranch = branchNames.find(b => b === "main") || 
                              branchNames.find(b => b === "master") || 
                              branchNames[0];
      ...
      toast({
        ...
        description: `Found ${branchNames.length} branches in the repository`,
      });
      ...
      toast({
        ...
        description: "Could not fetch branches. Using default options.",
        variant: "destructive",
      });
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => fetchBranches(repoUrl, accessToken)}
                      disabled={!repoUrl || fetchingBranches}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="access-token">Personal Access Token (Optional)</Label>
                  <Input
                    id="access-token"
                    type="password"
                ...
                <div>
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={fetchingBranches ? "Fetching branches..." : "Select branch"} />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  {availableBranches.length > 0 && (
                    <p className="text-xs text-muted-foreground mt-1">
                      {availableBranches.length} branches found
                    </p>
                  )}
                </div>
              </div>
                ...
                <TabsContent value="ec2" className="space-y-4 mt-4">
                  <Card className="bg-accent/20">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      {useExistingEC2 ? (
                        <div>
                          <Label htmlFor="instance-id">Select EC2 Instance from Your OneOps Account</Label>
                          <Select>
                            <SelectTrigger>
                              <SelectValue placeholder="Select an existing instance" />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="i-1234567890abcdef0">i-1234567890abcdef0 (Frontend Production - t3.medium)</SelectItem>
                              <SelectItem value="i-0987654321fedcba0">i-0987654321fedcba0 (API Service - t3.large)</SelectItem>
                              <SelectItem value="i-1111222233334444a">i-1111222233334444a (Worker Service - t3.small)</SelectItem>
                              <SelectItem value="i-5555666677778888b">i-5555666677778888b (Database Server - t3.xlarge)</SelectItem>
                              <SelectItem value="i-9999aaaabbbbcccc">i-9999aaaabbbbcccc (Staging Environment - t3.micro)</SelectItem>
                            </SelectContent>
                          </Select>
                          <p className="text-xs text-muted-foreground mt-1">
                            Only showing instances deployed through OneOps in your account
                          </p>
                        </div>
                      ) : (
                        <>

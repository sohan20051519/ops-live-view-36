...
const Sidebar = ({ activeItem = "dashboard", onItemSelect, className, isCollapsed = false, onToggleCollapse }: SidebarProps) => {
  ...
  return (
    <aside className={cn(
      "bg-card border-r border-border flex flex-col h-screen transition-all duration-300 overflow-hidden",
      isCollapsed ? "w-16" : "w-64",
      className
    )}>
      {/* Brand */}
      <div className={cn("p-6 border-b border-border flex-shrink-0", isCollapsed && "p-4")}>
      ...
      {/* Navigation */}
      <nav className="flex-1 p-4 space-y-2 overflow-hidden flex flex-col min-h-0">
